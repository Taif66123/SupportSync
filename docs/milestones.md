...
## Milestone 3 — Live Notifications — ✅ Phase A (Redis pub/sub + rate limiting remain)
...
**Built (Phase A, 107/107 tests green on sqlite + Postgres)**: `GET /api/v1/notifications/stream` (SSE; Bearer header or `?token=`; keep-alive comments; `event:`/`data:` frames of `{type, at, payload}`); typed events `ticket.created` / `ticket.updated` / `message.created`; recipients via declarative `notifications/policy.py` (all active Agents / conversation participants; admins never notified; actor subtracted); emissions **commit-gated** on the request session (`after_commit` publishes, `after_rollback` discards) so a subscriber never hears about unfetchable data; in-process hub fan-out (multi-stream per user); ephemeral by decision — no persistence, no replay.
