...
**Live Phase B smoke passed (2026-09-17, two uvicorn workers 8001/8002 + real SSE streams):** with Redis *down* — logins fail-open (~0.5 s), same-worker `ticket.updated` delivered, cross-worker frames correctly absent (negative control). With Redis *up* — the agent's 8001 stream received `ticket.created` for a ticket created on 8002, the customer's 8002 stream got both same-worker `ticket.updated` and cross-worker `message.created` (posted via 8001), and `PUBSUB NUMSUB = 2`: both listeners self-healed onto Redis after the breaker's 30 s cooldown, no restarts. Shared-window rate-limit proof: 3×401 then 429 on 8001, 6th attempt on 8002 → **429** (one Redis window across workers); keys verified via `redis-cli --scan` (`supportsync:ratelimit:auth-login:127.0.0.1`, `chat-send:<id>`, `ticket-create:<id>`).

**Phase B quality review ran** (`code-review` two-axis + thermo-nuclear): spec axis 100% (limits match the locked numbers, shared chat bucket REST+WS, origin dedupe, hermetic tests). Standards findings fixed: dead `NotificationOut` schema deleted; the twice-duplicated once-per-outage log machinery extracted to a shared `OutageLog` in `core/redis.py`; chat-send limit numbers centralized in `rate_limit_dependencies.py` (`CHAT_SEND_*` — REST and WS can no longer diverge); patching technique unified on module-attribute access. 118/118 re-verified on both engines after.
...
**Next entry points:** **M4** per `docs/milestones.md` and the notebook order — frontend (CORS already wired server-side), background processing (confirmation emails), multistage Dockerfile + compose packaging, CI (Jenkinsfile), QA pass. The Phase B live smoke and review are done (§5); the harness is current (AGENTS.md/CLAUDE.md through M3, `docs/database-design.md` is the DB doc of record).
